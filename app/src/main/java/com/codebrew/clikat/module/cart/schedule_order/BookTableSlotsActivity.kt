...
class BookTableSlotsActivity : BaseActivity<FragmentBookTableBinding, AgentViewModel>(), TabLayout.OnTabSelectedListener,
    ...
    private fun initialise() {
        ...
        if (intent != null) {
            if (intent?.hasExtra("supplierId") == true)
                supplierId = intent?.getStringExtra("supplierId")
            if (intent?.hasExtra("latitude") == true)
                latitude = intent?.getStringExtra("latitude")
            if (intent?.hasExtra("longitude") == true)
    ...
    private fun setAdapter() {
        ...
        // rv_timeslot?.isNestedScrollingEnabled = false
    ...
    private fun listeners() {
        ...
        tv_book.setOnClickListener {
            if (dateYYMMDD.isEmpty() || time24Format.isEmpty()) {
            ...
            } else if (mBinding?.etNumber?.text.toString().trim().isEmpty()) {
                viewDataBinding.root.onSnackbar(getString(R.string.enter_number_of_people))
            } else if (settingData?.table_book_mac_theme == "1" && intent?.hasExtra("seating_capacity") == true && intent?.hasExtra("supplierBranchId") == true) {
                holdSlotsApi()
            } else {
                ...
                val tempRequestHolder = hashMapOf(
                        ...
                        "branch_id" to intent?.getIntExtra("supplierBranchId", 0)
                )
...
*/
    private fun screenSelection() {
        ...
        if (datePosition != null && datePosition != -1 && supplierAvailableData != null) {
            ...
            intent.putExtra("branch_id",intent.getIntExtra("supplierBranchId", 0).toString())
            intent.putExtra("slot_id",supplierTimingsItem?.id?.toString())
    ...
    private fun getAgentSlots(date: String) {
        ...
        if (isNetworkConnected) {
            if (settingData?.table_book_mac_theme == "1" && intent?.hasExtra("seating_capacity") == true && intent?.hasExtra("supplierBranchId") == true) {
